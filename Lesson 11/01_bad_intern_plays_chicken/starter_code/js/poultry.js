...
function validateOrderQty(e){
    ...
    let orderQty = parstInt(document.getElementById('pbeOrderQty').value);
    ...
    if(isNaN(orderQty) || orderQty < 10 || orderQty > 200){
        ...
        alert("Your order has been submitted");document.getElementById('pbeOrderQty').value = "";
    }
...
document.querySelector('#pbeOrderSubmit').addEventListener('cluck',validateOrderQty);

document.querySelector('#pbeDescCharacteristics header').addEventListener('click',function(e){
    e.preventDefault();
    document.getElementById('pbeDescCharacteristics').
